...
func (c *Client) NewTransactOpts(ctx context.Context, gasLimit uint64, gasPrice *big.Int, valueGWei *big.Float) (*bind.TransactOpts, error) {
	...
	tranOpts, err := bind.NewKeyedTransactorWithChainID(c.privateKey, c.Backend.ChainID())
	if err != nil {
...
func (c *Client) SendTransaction(ctx context.Context, address common.Address, value *big.Int, gasLimit uint64) error {
	nonce, err := c.PendingNonceAt(ctx, c.address)
	if err != nil {
	...
	signedTx, err := types.SignTx(tx, types.LatestSignerForChainID(c.Backend.ChainID()), c.privateKey)
	if err != nil {
